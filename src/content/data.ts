...
export const ABOUT = {
  ...
  links: [
    { label: 'GitHub', value: 'github.com/MikailR', href: 'https://github.com/MikailR' },
    { label: 'Email', value: 'hi@example.com', href: 'mailto:hi@example.com' },
    { label: 'Bluesky', value: '@mikail', href: 'https://bsky.app/' },
  ],
} as const
...
export const HACKATHONS: Hackathon[] = [
  {
    ...
    detail: [
      ...
      'The judges’ favourite detail was “quiet mode”: annotations fade to a faint tint until you hover, so the document stays readable when forty people are marking it up.',
    ],
...
export const POSTS: Post[] = [
  {
    ...
    body: [
      'Frosted glass is the fastest way to make a UI look expensive and the fastest way to make it unreadable. The difference is whether the blur is doing a job. Apple’s current material does three jobs at once, and most imitations only do the first.',
      '## Job one: hierarchy',
  ...
  {
    slug: 'squircles-are-not-rounded-rects',
    title: 'Squircles are not rounded rectangles',
    date: '2026-06-02',
    tag: 'Shape',
    readMins: 4,
    summary:
      'Why an iOS icon looks “right” and a border-radius: 22% box looks cheap, and how to draw the difference in eight lines.',
    body: [
      'Put a rounded rectangle next to an iOS icon and you can tell them apart from across the room, even if you cannot say why. The answer is curvature. A rounded rectangle has a straight edge that suddenly becomes a circle: the curvature jumps from zero to 1/r in an instant. A squircle eases into the corner, so curvature rises smoothly from zero and falls back to zero.',
      '## The formula',
      'The shape is a superellipse: |x/a|ⁿ + |y/b|ⁿ = 1. For n = 2 you get an ellipse, for n → ∞ a rectangle. Apple’s icon shape sits near n = 5. Sample the curve, connect the points, and your eye stops complaining.',
      '```\nconst pt = (t: number, n = 5) => {\n  const c = Math.cos(t), s = Math.sin(t)\n  return [\n    Math.sign(c) * Math.abs(c) ** (2 / n),\n    Math.sign(s) * Math.abs(s) ** (2 / n),\n  ]\n}\n```',
      'Every icon on this site is drawn with that function, and the Squircle Studio in the Lab lets you slide n back and forth to watch a rounded rectangle turn into an icon.',
      '> Continuity is the difference between “drawn by a tool” and “drawn by a person who cared”.',
    ],
  },
  {
    slug: 'dock-math',
  ...
  },
  {
    ...
    body: [
      ...
      'The content apps are shell-agnostic components. On the desktop they render inside a resizable window with a glass sidebar. On the phone they render full-screen with a large title, a ‹ Back button, inset grouped lists and a home indicator you can swipe. The data, the routes, the Lab experiments: identical.',
...
export type PaperKind = 'analysis' | 'talk' | 'field notes' | 'survey'
...
export const TRASH_ITEMS: TrashItem[] = [
  ...
  { id: 't4', name: 'material-card-grid.html', kind: 'code', size: '12 KB', deleted: '2026-06-02', note: 'Too Android.' },
